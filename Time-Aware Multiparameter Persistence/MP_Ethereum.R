...
# Notes: Below code runs on Ethereum Token networks
############################################################################
...
######  RUNS ON ALL ETHEREUM EXPERIMENTS  ###### 
lisSC <- c()
lisSC <- c(lisSC, 'bytom') # ### 
...
for (nomNetGraph in lisSC) {
  ...
  datasetSplit = strsplit(dataAlias,'_') 
  if(datasetSplit[[1]][1]=='bytom'){
    totalNets = 285  
    #totalNets = 7
  }else if(datasetSplit[[1]][1]=='decentraland'){
    totalNets = 206  
    #totalNets = 7
  }else if(datasetSplit[[1]][1]=='golem'){
    totalNets = 443  
    #totalNets = 7
  }
