...
# Notes: Below code runs on COVID-19 networks
############################################################################
library(igraph) 
library(TDA)
library(reticulate) 
...
computePersistentGrid<-function(indexLow, indexTop, dataset, dataAlias, feature1, feature2, typeFiltrationF2=1){
  ...
  } else if((dataset=='CA_001') || (dataset=='CA_005') || (dataset=='TX_001') || (dataset=='TX_005')){ # COVID-19
    datasetSplit = strsplit(dataset,'_')   
    matNets <- np$load(paste0("Datasets/COVID19/",datasetSplit[[1]][1],"_Covid19_ADJ_",datasetSplit[[1]][2],".npy"), allow_pickle=TRUE)   
    graphs = 1:length(matNets)
    
  } else { # OTHER DATASETS (previous)
...
######  RUNS ON ALL COVID-19 EXPERIMENTS  ###### 
lisSC <- c()
lisSC <- c(lisSC, 'CA_001')  
lisSC <- c(lisSC, 'CA_005')  
lisSC <- c(lisSC, 'TX_001')  
lisSC <- c(lisSC, 'TX_005')  
...
for (nomNetGraph in lisSC) {
  ...
  totalNets = 335 # Number of days for covid data 
  #lenBlock = totalNets  # To compute all at once...
