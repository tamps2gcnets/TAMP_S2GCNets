...
from MPGCNCELL import MPGCRNCNNCell


class MPDCRNNCNN(nn.Module):
    def __init__(self, node_num, dim_in, dim_out, link_len, embed_dim, num_layers=1, window_len = 12, fixed_adj = 0, adj = None, stay_cost = 0.1, jump_cost = 0.2):
        super(MPDCRNNCNN, self).__init__()
        ...
        for _ in range(0, num_layers-1):
            self.nlsdcrnncnn_cells.append(MPGCRNCNNCell(node_num, dim_in, dim_out, window_len, link_len, embed_dim))
        for _ in range(num_layers-1, num_layers):
            self.nlsdcrnncnn_cells.append(MPGCRNCNNCell(node_num, dim_out, dim_out, window_len, link_len, embed_dim))
...
# ************************************************************** #
class MPGCRNCNN(nn.Module):
    def __init__(self, args):
        super(MPGCRNCNN, self).__init__()
        ...
        self.encoder = MPDCRNNCNN(args.num_nodes, args.input_dim, args.rnn_units, args.link_len,
