...
class TAMPSTGCNCNN(nn.Module):
    def __init__(self, dim_in, dim_out, window_len, link_len, embed_dim):
        ...
        self.cnn_mean = MEANCNN(1, int(dim_out / 8))
        self.cnn_max = MAXCNN(1, int(dim_out / 8))
    ...
    def forward(self, x, x_window, node_embeddings, fixed_adj, adj, stay_cost, jump_cost, MPG):
        ...
        mean_MPG = torch.mean(MPG, dim=1)  #(B, MPG_dim, MPG_dim)
        mean_MPG = mean_MPG.unsqueeze(dim = 1)
        
        topo_mean_cnn = self.cnn_mean(mean_MPG) #(B, dim_out/8)
        topo_max_cnn = self.cnn_max(mean_MPG)  #(B, dim_out/8)
