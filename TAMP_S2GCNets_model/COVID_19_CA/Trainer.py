...
class Trainer(object):
    ...
    def val_epoch(self, epoch, val_dataloader):
        ...
        with torch.no_grad():
            for batch_idx, (ori_data, MPG, target) in enumerate(val_dataloader):
    ...
    def train_epoch(self, epoch):
        self.model.train()
        total_loss = 0
        for batch_idx, (ori_data, MPG, target) in enumerate(self.train_loader):
    ...
    def test(model, args, data_loader, scaler, logger, path=None):
        ...
        y_true = []
        with torch.no_grad():
            for batch_idx, (ori_data, MPG, target) in enumerate(data_loader):
                data = ori_data[..., :args.input_dim]
                #x_full = ori_data[..., :args.input_dim]
                label = target[..., :args.output_dim]
                output = model(data, target, MPG, teacher_forcing_ratio=0)
                y_true.append(label)
