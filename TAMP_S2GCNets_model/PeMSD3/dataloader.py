...
def get_topo_dataloader(args, data_type, normalizer = 'std', tod=False, dow=False, weather=False, single=False):
    ...
    data, scaler = normalize_dataset(data, normalizer, args.column_wise)
    #spilit dataset by days or by ratio
    topo_data = load_topo_dataset(data_type)

    if args.test_ratio > 1:
        data_train, data_val, data_test = split_data_by_days(data, args.val_ratio, args.test_ratio)
        topo_data_train, topo_data_val, topo_data_test = split_data_by_days(topo_data, args.val_ratio, args.test_ratio)
    else:
        data_train, data_val, data_test = split_data_by_ratio(data, args.val_ratio, args.test_ratio)
        topo_data_train, topo_data_val, topo_data_test = split_data_by_ratio(topo_data, args.val_ratio, args.test_ratio)

    single = False
    x_tra, y_tra = Add_Window_Horizon(data_train, args.lag, args.horizon, single)
    topo_tra = MPG_Window_Horizon(topo_data_train, args.lag, args.horizon, single)
    x_val, y_val = Add_Window_Horizon(data_val, args.lag, args.horizon, single)
    topo_val = MPG_Window_Horizon(topo_data_val, args.lag, args.horizon, single)
    x_test, y_test = Add_Window_Horizon(data_test, args.lag, args.horizon, single)
    topo_test = MPG_Window_Horizon(topo_data_test, args.lag, args.horizon, single)
    print('Train: x, MPG, y ->', x_tra.shape, topo_tra.shape, y_tra.shape)
    print('Val: x, MPG, y ->', x_val.shape, topo_val.shape, y_val.shape)
    print('Test: : x, MPG, y ->', x_test.shape, topo_test.shape, y_test.shape)
